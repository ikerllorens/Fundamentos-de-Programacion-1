...
extern void listado(catalogo disco[])
{
	...
	switch(opc)
	{
		case 1: //Impresión mediante número de disco
		{
			while(1)
			{	
				if(disco[indice].estado==0)//Salta discos vacios
				{
					if(opc2==1)
					{
						indice--;
				...
				scanf("%d",&opc2);
				if(opc2!=1 && opc2!=2 & opc2!=3)//Validacion APP
				{
					system("clear");
...
void imprimeListadoString(catalogo discoCmp[])
{
	...
	while(1)
	{
		if(discoCmp[i].estado==0 && i<SIZE)//Salta discos vacios.
		{
			if(opc2==1)
			{
				i--;
